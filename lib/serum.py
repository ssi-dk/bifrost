        ...
        with open(os.path.join(sample, "sample.yaml"), "w") as sample_yaml:
            if sample in config['serum']['samples_to_ignore']:
                ...
                with open(os.path.join(sample, "cmd"), "w") as command:
                    command.write("snakemake -s ~/git.repositories/SerumQC-private/serumqc.snake --config R1_reads={} R2_reads={} Sample={}".format(run_info["samples"][sample]["R1"], run_info["samples"][sample]["R2"]), sample + "/sample.yaml")
