                        ...
                        with open(sample_name + "/cmd_" + component + "_{}.sh".format(current_time), "w") as command:
                            ...
                            for component_name in components:
                                ...
                                if sample_name in config["samples_to_ignore"]:
                                    ...
                                    command.write("snakemake {} --use-singularity  --singularity-args \"{}\" --singularity-prefix \"/srv/data/tools/singularity\" --restart-times {} --cores {} -s {} {} --config Sample={}; \n".format(tmp_dir, "-B " + sample_db["reads"]["R1"] + "," + sample_db["reads"]["R2"] + "," + os.getcwd() + "," + os.path.dirname(os.getenv("BIFROST_DB_KEY")), config["restart_times"], config["threads"], component_file, unlock, "sample.yaml"))
