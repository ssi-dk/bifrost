...
Create a file named `keys.txt` with the mongodb URI including the database to use. You'll also need a `species_keys.txt` with the uri to the species collection (can be the same as the previous)

Create an environment variable with the mongodb keys locations.
```
export BIFROST_DB_KEY=/path/to/keys.txt
export BIFROST_SPECIES_DB_KEY=/path/to/keys.txt
```
